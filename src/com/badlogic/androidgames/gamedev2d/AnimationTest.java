	...
	class AnimationScreen extends Screen{		
		...
		public AnimationScreen(Game game) {
			...
			for(int i= 0;i<NUM_CAVEMEN;i++){
				cavemen[i] = new Caveman((float)Math.random(),(float)Math.random(),0.2f,1);
			}
		...
		@Override
		public void resume() {
			texture = new Texture(((GLGame)game), "dir/texture/ball.png");
			walkAnim = new Animation(0.2f, new TextureRegion(texture, 0, 0, 42, 42),
										   new TextureRegion(texture, 42, 0, 42, 42),
										   new TextureRegion(texture, 84, 0, 42, 42));
